...
export default function SellerOrdersPage() {
  ...
  if (!user) return (
    <div className="p-8">
      <p>Please sign in to view seller orders.</p>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-neutral-bg py-14 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto space-y-8">
        <div>
          <p className="text-sm uppercase tracking-[0.3em] text-neutral-muted">Store Orders</p>
          <h1 className="mt-3 text-3xl font-extrabold text-neutral-text">Orders for your products</h1>
          <p className="mt-2 text-sm text-neutral-muted">Orders containing items from your store.</p>
        </div>
        ...
        {data.length === 0 ? (
          <div className="rounded-3xl border border-neutral-border bg-white p-10 text-center">
            ...
            <p className="mt-3 text-sm text-neutral-muted">Orders that contain your products will appear here.</p>
            <Link href="/sellers/products" className="mt-6 inline-flex items-center justify-center rounded-full bg-primary px-6 py-3 text-sm font-semibold text-white hover:bg-primary-hover transition-colors">Manage products</Link>
          </div>
        ) : (
              ...
              <div key={order.id} className="rounded-3xl border border-neutral-border bg-white p-6 shadow-sm">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-neutral-muted">Placed: {new Date(order.created_at).toLocaleString()}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-neutral-muted">Status</p>
                    <p className="font-semibold text-neutral-text capitalize">{order.status}</p>
                    <p className="text-sm text-neutral-muted">Order total</p>
                ...
                <div className="mt-4 space-y-2">
                  {items.map((it) => (
                    <div key={it.id} className="flex items-center justify-between">
                      <div>
                        <p className="text-sm text-neutral-text">Product: {it.product?.title ?? it.product_id}</p>
                        <p className="text-xs text-neutral-muted">Quantity: {it.quantity}</p>
                        <p className="text-xs text-neutral-muted">Fulfillment: {it.seller_status || 'pending'}</p>
                      </div>
                      <div className="flex items-center gap-4">
                        <div className="text-sm font-semibold">{(it.unit_price ?? 0).toLocaleString()} FCFA</div>
