...
import { Phone, Loader2, CheckCircle2, Smartphone, AlertCircle, ArrowLeft, CreditCard } from 'lucide-react';
...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    const loadCart = async () => {
      ...
      try {
        ...
        setError(err?.message || t('loadCartError') || 'Unable to load your cart.');
      }
    };
  ...
  useEffect(() => {
    ...
    if (paymentStep === 'pending' && paymentReference) {
      interval = setInterval(async () => {
        try {
          ...
          if (data.status === 'SUCCESSFUL') {
            ...
            // Once paid, create the order
            await finalizeOrder(paymentReference);
          } else if (data.status === 'FAILED') {
            setPaymentStep('failed');
            setBusy(false);
            setError('Payment failed or was canceled by user.');
            clearInterval(interval);
          }
        } catch (err) {
  ...
  const handlePlaceOrder = async () => {
    ...
    if (!formattedPhone) {
      setError('Please enter a valid Cameroon phone number (e.g. 6xxxxxxxx)');
      return;
    }
  ...
  const finalizeOrder = async (reference: string) => {
    if (!user) return;
    try {
      await createOrderFromCart(user.id, items, reference);
      setSuccess(t('orderPlaced'));
      setTimeout(() => router.push('/products'), 3000);
    } catch (err: any) {
      setError('Payment was successful, but there was an error creating your order. Please contact support.');
    ...
    return (
      <div className="min-h-screen bg-neutral-bg py-14 px-4 sm:px-6 lg:px-8 flex items-center justify-center">
        <div className="max-w-md w-full bg-white border border-emerald-100 rounded-3xl p-10 text-center shadow-xl animate-in zoom-in duration-500">
          ...
          <div className="mt-8 p-4 bg-neutral-bg rounded-2xl flex items-center gap-3">
            <Loader2 className="w-4 h-4 animate-spin text-primary" />
            <p className="text-xs font-bold text-neutral-muted uppercase tracking-widest">{t('redirecting')}</p>
          </div>
        </div>
      </div>
    );
                ...
                <div key={item.id} className="rounded-3xl border border-neutral-border bg-white p-5 shadow-sm group hover:shadow-md transition-shadow">
                  <div className="flex items-center gap-5">
                    ...
                    <div className="flex-1 min-w-0">
                      <h2 className="text-base font-bold text-neutral-text truncate">{item.product?.title}</h2>
          ...
          <aside className="space-y-6">
            <div className="rounded-3xl border border-neutral-border bg-white p-6 shadow-sm sticky top-24">
              ...
              <p className="mt-4 text-[10px] text-neutral-muted text-center leading-relaxed">
                By clicking "{t('placeOrder')}", you agree to our terms. Your payment is processed securely by Campay.
              </p>
            </div>
          </aside>
        </div>
