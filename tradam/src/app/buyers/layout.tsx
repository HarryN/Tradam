...
function BuyerSidebarContent({
  ...
  // Real-time order notification for buyer
  useEffect(() => {
    if (!user) return;
    ...
    const loadCount = async () => {
      try {
        ...
        console.error('Error loading buyer order count:', err);
      }
    };

    loadCount();

    // Use a truly unique channel ID for every mount to avoid the "already subscribed" error
    const channelId = `buyer-notif-${user.id.slice(0, 8)}-${Math.random().toString(36).substring(7)}`;
  ...
  const navLinks: BuyerNavLink[] = useMemo(() => [
    ...
    { href: '/buyers/orders', labelKey: 'orders', icon: ShoppingBag, badge: orderCount > 0 ? orderCount.toString() : undefined },
    { href: '/buyers/reviews', labelKey: 'recommendations', icon: Star },
  ], [orderCount]);
