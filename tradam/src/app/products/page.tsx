        ...
        <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <div className="mt-3 text-xs text-neutral-muted">
              {products.length} {t('activeProducts')} {activeCategory ? ` in ${t(activeCategory.name)}` : ''}.
            </div>
          </div>
              ...
              <Link key={p.id} href={`/products/${p.id}`} className="group block bg-white border border-neutral-border rounded-3xl overflow-hidden shadow-sm hover:shadow-lg transition-all">
                <div className="relative h-60 bg-neutral-bg flex items-center justify-center overflow-hidden">
                  ...
                  <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 via-black/10 to-transparent p-4">
                    <p className="text-xs uppercase tracking-[0.18em] text-white/80">{t(p.category?.name || 'uncategorized')}</p>
                  </div>
                </div>
                <div className="p-5">
                  <h2 className="text-base font-semibold text-neutral-text line-clamp-2">{tc(p.title)}</h2>
                  <p className="mt-3 text-sm text-neutral-muted line-clamp-2">{tc(p.description)}</p>
                  <div className="mt-5 flex items-center justify-between gap-3">
                    <span className="font-extrabold text-neutral-text">{p.price.toLocaleString()} {t('priceCurrency')}</span>
                    <span className="text-xs text-neutral-muted">{p.stock} {t('stockAvailable')}</span>
                  </div>
                </div>
              </Link>
            ))}
