      ...
      {items.length === 0 ? (
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_350px]">
          <div className="space-y-4">
            {items.map((item) => (
              <div key={item.id} className="bg-white p-5 rounded-3xl border border-neutral-border shadow-sm group hover:border-primary/20 transition-all">
                <div className="flex flex-col sm:flex-row gap-6">
                  <div className="w-24 h-24 rounded-2xl bg-neutral-bg overflow-hidden border border-neutral-border shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start gap-4">
                      <div>
                        <h4 className="text-lg font-bold text-neutral-text truncate">{tc(item.product?.title)}</h4>
                        <p className="text-xs font-bold text-primary mt-1 uppercase tracking-wider">{t(item.product?.category?.name || 'others')}</p>
                      </div>
                      <p className="text-lg font-black text-neutral-text whitespace-nowrap">
                    ...
                    <div className="mt-6 flex flex-wrap items-center justify-between gap-4">
                      <div className="flex items-center gap-1 bg-neutral-bg rounded-xl p-1 border border-neutral-border">
                        ...
                        <span className="w-8 text-center text-sm font-black text-neutral-text">{item.quantity}</span>
                        <button 
                          onClick={() => handleQuantity(item, item.quantity + 1)}
                          disabled={saving}
                      ...
                      <button 
                        ...
                        disabled={saving}
                        className="flex items-center gap-2 text-xs font-bold text-red-500 hover:text-red-600 uppercase tracking-widest px-3 py-2 rounded-xl hover:bg-red-50 transition-all"
                      >
          ...
          <aside>
            <div className="bg-white p-8 rounded-3xl border border-neutral-border shadow-sm sticky top-24 space-y-8">
